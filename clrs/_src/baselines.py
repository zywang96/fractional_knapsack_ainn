...
class BaselineModel(model.Model):
  ...
  def _predict(self, params, rng_key: hk.PRNGSequence, features: _Features,
    ...
    #print(outs['pred'])
    outs1 = decoders.postprocess(self._spec[algorithm_index],
                                outs,
                                sinkhorn_temperature=0.1,
                                sinkhorn_steps=50,
                                hard=True,
                                )
    outs_soft = decoders.postprocess(self._spec[algorithm_index],
                                outs,
                                sinkhorn_temperature=0.1,
                                sinkhorn_steps=50,
                                hard=False, inter = True
                                )

    if self.debug:
      return outs1, hint_preds, hidden_states
    else:
      return outs1, hint_preds, outs_soft
